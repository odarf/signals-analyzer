...
float MainWindow::embedRandom(){ //перенёс в класс modeling
    int max = 1;
    int min = -1;
...
float MainWindow::randomGenerator(float coeff){ //перенёс в класс modeling
...
switch(currentTask){
    ...
    // ---------------- Task 5 ----------------
        ...
        double kHist = analysis.minValue(ySin);
        double shag = (qAbs(analysis.minValue(ySin)) + analysis.maxValue(ySin))/M;
    ...
    // ---------------- Task 7 ----------------
        int offsetCoeff = 5;
        QVector<double> offsetted = processing.offset(ySin, offsetCoeff);
        QVector<double> spiked = processing.spikes(ySin);
